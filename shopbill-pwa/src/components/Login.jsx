...
// --- Custom Phone Number Validation (Kept for Forgot Password / future use, though not used in current login flow) ---
/**
...
// --- Sub-Component: Login Form (UPDATED to use onBackToLanding for Create Account) ---
const LoginForm = ({ handleAuth, identifier, setIdentifier, password, setPassword, loading, setView, authError, onBackToLanding }) => {
    ...
    return (
        <>
            ...
            {/* Switch View to Landing Page (for Signup) */}
            <p className="text-center text-gray-600 dark:text-gray-400 mt-6 pt-4 border-t border-gray-200 dark:border-gray-700/50">
                New to Pocket POS?
                <button
                    onClick={onBackToLanding} // This function should now be configured in the parent to navigate to http://localhost:5173/#pricing
    ...
    return (
        <>
            ...
            {resetMessage && resetMessage.success && (
                <div className="p-3 mb-4 text-sm text-green-100 bg-green-600 dark:bg-green-800 rounded-lg text-center" role="alert">
            ...
            {resetMessage && resetMessage.error && (
                <div className="p-3 mb-4 text-sm text-red-100 bg-red-600 dark:bg-red-800 rounded-lg text-center" role="alert">
                    {resetMessage.error}
                </div>
            )}

            <form className="space-y-4" onSubmit={handleForgotPasswordRequest}>
                <p className="text-gray-600 dark:text-gray-400 text-sm text-center pt-2">
...
const Login = ({ onLogin, onBackToLanding, initialPlan }) => {
    ...
    // Updated Email Change Handler (Used only for Forgot Password email field)
    ...
    // New Email Blur Handler (Used only for Forgot Password email field)
    const handleEmailBlur = useCallback(() => {
        // Only validate if in a view that strictly requires email (ForgotPassword)
    ...
    useEffect(() => {
        ...
        // Reset password input when switching away from login
        if (view !== 'login') {
            setPassword('');
        }
    }, [view]);
    ...
    const handleAuth = async (e) => {
        ...
        // **LOGIN VIEW:** Simple required field check
        if (!identifier) {
        ...
        // Login sends 'identifier' (email or phone) and 'password'
            ...
            default:
                return (
                    <LoginForm
                        ...
                        onBackToLanding={onBackToLanding} // ADDED: For "Create Account" button redirect
                    />
                );
        }
    };
