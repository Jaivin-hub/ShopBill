                                ...
                                return (
                                    ...
                                    >
                                        <div className="flex items-center gap-4 md:gap-5 flex-1 min-w-0">
                                            <div className={`w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl flex items-center justify-center border shrink-0 ${getRoleStyles(s.role)}`}>
                                                {s.role === 'owner' ? <Crown className="w-5 h-5 md:w-6 md:h-6" /> : <User className="w-5 h-5 md:w-6 md:h-6" />}
                                            </div>
                                            <div className="min-w-0 flex-1">
                                                <h3 className={`text-base md:text-lg font-black tracking-tight truncate ${darkMode ? 'text-white' : 'text-slate-900'}`}>{s.name}</h3>
                                                <p className={`text-[11px] md:text-xs font-bold truncate mt-0.5 ${darkMode ? 'text-slate-400' : 'text-slate-500'}`}>{s.email}</p>
                                                <div className="flex items-center gap-2 mt-2">
                                                    <span className={`text-[8px] md:text-[9px] font-black px-2 py-0.5 rounded border tracking-widest uppercase ${getRoleStyles(s.role)}`}>
                                                        {s.role}
                                                    </span>
                                                </div>
                                            </div>
                                        ...
                                        <div className={`flex items-center gap-2 md:gap-3 w-full sm:w-auto mt-4 sm:mt-0 pt-4 sm:pt-0 border-t sm:border-0 ${borderStyle}`}>
                                            <button
                                                ...
                                                className={`flex-1 sm:flex-none px-4 md:px-5 py-2.5 md:py-3 rounded-xl md:rounded-2xl font-black text-[9px] md:text-[10px] tracking-widest transition-all active:scale-95 flex items-center justify-center gap-2 ${
                                                    s.active 
