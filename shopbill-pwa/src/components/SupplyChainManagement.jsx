...
const SupplyChainManagement = ({ apiClient, API, showToast, darkMode }) => {
  ...
  return (
    <div className={`min-h-screen ${themeBase} selection:bg-indigo-500/30 font-sans transition-colors duration-200`}>
        ...
        {activeTab === 'purchase' && (
          <div className="animate-in fade-in slide-in-from-bottom-2 duration-500">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-stretch">
                ...
                <div className="space-y-4">
                    ...
                    <div className="space-y-1.5">
                      ...
                      <div className="flex gap-2">
                        <button type="button" onClick={() => { setIsProductPickerOpen(true); setSearchTerm(''); }} className={`flex-1 ${inputBase} px-4 py-3 rounded-xl flex items-center justify-between text-left hover:border-indigo-500/50 transition-all`}>
                          <span className={`text-[11px] font-bold uppercase truncate ${selectedProduct ? (darkMode ? 'text-white' : 'text-slate-900') : 'text-gray-400'}`}>{selectedProduct ? selectedProduct.name : 'Choose Item...'}</span>
                    ...
                    <div className="space-y-1.5">
                      ...
                      <div className="flex gap-2">
                        <button type="button" onClick={() => { setIsSupplierPickerOpen(true); setSearchTerm(''); }} className={`flex-1 ${inputBase} px-4 py-3 rounded-xl flex items-center justify-between text-left hover:border-indigo-500/50 transition-all`}>
                          <span className={`text-[11px] font-bold uppercase truncate ${selectedSupplier ? (darkMode ? 'text-white' : 'text-slate-900') : 'text-gray-400'}`}>{selectedSupplier ? selectedSupplier.name : 'Choose Vendor...'}</span>
                          <ChevronDown className="w-4 h-4 text-gray-500" />
                        </button>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-1.5">
                      ...
                      <input type="date" value={purchaseForm.date} onChange={(e) => setPurchaseForm({ ...purchaseForm, date: e.target.value })} className={`w-full ${inputBase} px-4 py-3 rounded-xl outline-none text-[10px] font-bold`} />
                    </div>
                  </div>
                </div>
              ...
              <div className={`${darkMode ? 'bg-gray-900/20 border-gray-800' : 'bg-white border-slate-200 shadow-sm'} rounded-2xl overflow-hidden flex flex-col`}>
                <div className={`p-4 border-b ${darkMode ? 'bg-gray-900/50 border-gray-800' : 'bg-slate-50 border-slate-100'} space-y-3`}>
                  <div className="flex justify-between items-center">
                    <h3 className={`text-[10px] font-black uppercase tracking-widest ${darkMode ? 'text-white' : 'text-slate-900'}`}>Live Stock Overview</h3>
                    <div className="flex items-center gap-2">
                      <select
                        value={inventorySort}
                        onChange={(e) => setInventorySort(e.target.value)}
                        className={`${darkMode ? 'bg-gray-950 border-gray-800' : 'bg-white border-slate-200 shadow-sm'} text-[9px] font-black uppercase text-indigo-400 px-2 py-1 rounded-lg outline-none cursor-pointer`}
                      >
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className={`w-full ${inputBase} py-2 pl-9 pr-4 rounded-xl text-[10px] outline-none focus:border-indigo-500 transition-all`}
                    />
                  </div>
                </div>
                  ...
                  {sortedInventory.map(item => {
                    ...
                    return (
                      <button
                        ...
                        className={`p-4 rounded-xl border text-left transition-all relative group hover:scale-[1.02] active:scale-95 ${isSelected ? 'bg-indigo-600/20 border-indigo-500 shadow-lg' :
                            isLow ? 'bg-red-500/5 border-red-500/20' : (darkMode ? 'bg-gray-950/50 border-gray-800/50' : 'bg-slate-50 border-slate-200')
                          }`}
                      >
                        <div className="flex justify-between items-start mb-2 gap-2">
                          <p className={`text-[10px] font-black uppercase truncate ${isSelected ? 'text-indigo-400' : (darkMode ? 'text-gray-400' : 'text-slate-600')}`}>{item.name}</p>
          ...
          <div className="flex flex-col h-[calc(100vh-180px)] animate-in fade-in duration-500 space-y-4">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 shrink-0">
              {[
                ...
                <div key={i} className={`${darkMode ? 'bg-gray-900/50 border-gray-800 group hover:bg-gray-900' : 'bg-white border-slate-200 group hover:bg-slate-50'} border p-4 rounded-xl flex flex-col items-center text-center transition-all shadow-sm`}>
                  <div className={`p-2 ${darkMode ? 'bg-gray-950 border-gray-800' : 'bg-slate-100 border-slate-200'} rounded-lg mb-2 border group-hover:border-indigo-500/30 transition-all`}><stat.icon className="w-4 h-4 text-indigo-500" /></div>
                  <p className="text-[8px] font-black text-gray-600 uppercase tracking-widest mb-1">{stat.label}</p>
                  <p className={`text-sm font-black ${darkMode ? 'text-white' : 'text-slate-900'}`}>{stat.val}</p>
                </div>
              ))}
            </div>
            ...
            <div className={`${darkMode ? 'bg-gray-900/20 border-gray-800' : 'bg-white border-slate-200 shadow-sm'} rounded-2xl flex flex-col overflow-hidden min-h-0`}>
              <div className={`p-4 ${darkMode ? 'bg-gray-900/50 border-gray-800' : 'bg-slate-50 border-slate-100'} border-b flex flex-col md:flex-row justify-between items-center px-6 gap-4 shrink-0`}>
                <div className="flex gap-2 overflow-x-auto no-scrollbar max-w-full">
                  {DATE_FILTERS.map(f => (
                    <button key={f.id} onClick={() => setSelectedFilter(f.id)} className={`px-4 py-2 rounded-lg text-[9px] font-black uppercase transition-all whitespace-nowrap ${selectedFilter === f.id ? 'bg-indigo-600 text-white shadow-lg' : 'text-gray-500 hover:bg-slate-200 dark:hover:bg-gray-800'}`}>{f.label[0]} {f.label[1]}</button>
                  ))}
                </div>
                {selectedFilter === 'custom' && (
                  <div className="flex items-center gap-2">
                    <input type="date" value={customStartDate} onChange={(e) => setCustomStartDate(e.target.value)} className={`${inputBase} text-[10px] font-bold p-2 rounded-lg outline-none`} />
                    <input type="date" value={customEndDate} onChange={(e) => setCustomEndDate(e.target.value)} className={`${inputBase} text-[10px] font-bold p-2 rounded-lg outline-none`} />
                  </div>
                )}
              </div>
              <div className="overflow-auto custom-scrollbar flex-1 min-h-0">
                <table className="w-full text-left min-w-[700px]">
                  <thead
                    className={`sticky top-0 z-10 ${darkMode ? 'bg-gray-900' : 'bg-slate-100'
                      } text-[9px] font-black text-gray-500 uppercase tracking-widest border-b ${darkMode ? 'border-gray-800' : 'border-slate-200'
                      }`}
                  >
                    <tr>
                  ...
                  <tbody
                    className={`divide-y ${darkMode ? 'divide-gray-800/30' : 'divide-slate-100'
                      } text-[10px]`}
                  >
                    {filteredHistory.map((record) => (
                      <tr
                        key={record._id}
                        className={`${darkMode ? 'hover:bg-white/[0.02]' : 'hover:bg-slate-50'
                      ...
                      >
                        <td className="px-6 py-4">
                          ...
                          <p className="text-[8px] font-mono text-gray-500 mt-0.5 uppercase">
                        ...
                        <td className="px-6 py-4">
                          <p
                            className={`font-black uppercase ${darkMode ? 'text-white' : 'text-slate-900'
                              }`}
                          >
                        ...
                        <td className="px-6 py-4 text-center">
                          <span className="bg-indigo-500/10 text-indigo-500 px-2 py-1 rounded font-black">
                            +{record.quantity}
                          </span>
                        </td>
        ...
        {activeTab === 'suppliers' && (
          <div className="space-y-6 animate-in fade-in duration-500">
            <div className="flex justify-end items-center pb-2">
              <button onClick={() => setIsSupplierModalOpen(true)} className={`${darkMode ? 'bg-white text-black hover:bg-gray-200' : 'bg-slate-900 text-white hover:bg-slate-800'} px-5 py-2 rounded-xl text-[10px] font-black uppercase transition-all active:scale-95 shadow-xl flex items-center gap-2`}>
                ...
                <div key={s._id} className={`${cardBase} p-4 rounded-xl group hover:border-indigo-500/50 transition-all flex flex-col justify-between`}>
                  <div>
                    <div className="flex justify-between items-start mb-3">
                      <div className="w-8 h-8 bg-indigo-500/10 rounded-lg flex items-center justify-center text-indigo-500 font-black text-sm border border-indigo-500/10 uppercase">{s.name ? s.name[0] : 'V'}</div>
                      <ExternalLink className={`w-3 h-3 text-gray-500 group-hover:text-indigo-500 transition-all cursor-pointer`} />
                    </div>
                    <h4 className={`text-[11px] font-black uppercase truncate mb-3 ${darkMode ? 'text-white' : 'text-slate-900'}`}>{s.name}</h4>
      ...
      {isProductModalOpen && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
          ...
          <form onSubmit={handleQuickAddProduct} className={`relative w-full max-w-sm ${darkMode ? 'bg-gray-900 border-gray-800' : 'bg-white border-slate-200'} p-6 rounded-2xl border shadow-2xl animate-in zoom-in duration-200`}>
            <h3 className={`text-lg font-black uppercase tracking-tighter mb-4 flex justify-between items-center ${darkMode ? 'text-white' : 'text-slate-900'}`}>
          ...
          <div className={`relative w-full max-w-md ${darkMode ? 'bg-gray-900 border-gray-800' : 'bg-white border-slate-200'} rounded-2xl border max-h-[75vh] flex flex-col overflow-hidden animate-in zoom-in duration-200 shadow-2xl`}>
            <div className={`p-5 border-b ${darkMode ? 'bg-gray-900/50 border-gray-800' : 'bg-slate-50 border-slate-100'}`}>
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-[10px] font-black uppercase tracking-widest text-indigo-500">{isProductPickerOpen ? 'Select Product' : 'Select Vendor'}</h3>
              ...
              <div className="relative">
                ...
                <input autoFocus placeholder="Type to search..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className={`w-full ${inputBase} p-3 pl-10 rounded-xl outline-none text-xs focus:border-indigo-500`} />
              </div>
            </div>
              ...
              {(isProductPickerOpen ? filteredProducts : filteredSuppliers).map(item => {
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center justify-between p-3.5 hover:bg-indigo-600 rounded-xl transition-all group border border-transparent`}
                  >
                    <div className="text-left">
                      <p className={`text-xs font-bold uppercase truncate ${darkMode ? 'text-gray-200 group-hover:text-white' : 'text-slate-700 group-hover:text-white'}`}>{item.name}</p>
                      {isProductPickerOpen && <p className={`text-[8px] font-black uppercase mt-0.5 ${isLow ? 'text-red-500 group-hover:text-white' : 'text-gray-500 group-hover:text-indigo-100'}`}>Stock: {item.quantity}</p>}
                    </div>
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
          ...
          <form onSubmit={handleAddSupplier} className={`relative w-full max-w-sm ${darkMode ? 'bg-gray-900 border-gray-800' : 'bg-white border-slate-200'} p-6 rounded-2xl border shadow-2xl animate-in zoom-in duration-300`}>
            ...
            <div className="space-y-4">
              ...
              <input placeholder="Tax ID / GSTIN (Optional)" value={supplierForm.gstin} onChange={e => setSupplierForm({ ...supplierForm, gstin: e.target.value })} className={`w-full ${inputBase} p-3 rounded-xl outline-none text-[10px] font-mono uppercase text-indigo-500`} />
              <button className={`w-full mt-2 ${darkMode ? 'bg-white text-black hover:bg-gray-200' : 'bg-slate-900 text-white hover:bg-slate-800'} py-3 rounded-xl font-black uppercase text-[10px] tracking-widest transition-all shadow-xl`}>Save Vendor</button>
            </div>
          </form>
        </div>
      )}
