        ...
        <main className="p-4 md:p-8 h-full overflow-y-auto bg-gray-950 transition-colors duration-300" itemScope itemType="https://schema.org/ItemList">
            <header className="flex justify-between items-center mb-6" itemProp="headline">
                ...
                {notifications.length > 0 && (
                    <button 
                        ...
                        className="text-xs font-bold uppercase tracking-wider text-red-500 hover:text-white hover:bg-red-600 transition-all bg-red-900/20 px-4 py-2 rounded-lg border border-red-900/50 outline-none active:scale-95"
                    >
                        Clear All
                    </button>
                )}
            ...
            <header className="space-y-3 pb-24 md:pb-12">
                {notifications.length > 0 ? (
                    notifications.map((notification, index) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className={`flex-shrink-0 mr-4 ${color} p-2 bg-gray-900 rounded-lg shadow-sm`}>
                                    <Icon className="w-5 h-5" />
                                </div>
                                <div className="flex-grow">
                                    <div className="flex justify-between items-start">
                                        <p className={`text-gray-100 leading-snug ${isNew ? 'font-bold' : 'font-medium'}`}>
                                            {notification.message}
                                        </p>
                                    </div>
                                    <div className="flex items-center mt-2 space-x-3">
                                        <span className={`text-[9px] px-2 py-0.5 rounded-full uppercase font-black border ${borderColor} ${color} bg-black/40`}>
                                ...
                                <button
                                    ...
                                    className="ml-4 p-2 rounded-lg hover:bg-red-900/30 text-gray-400 hover:text-red-500 transition-colors"
                                >
                                    <X className="w-4 h-4" />
                                </button>
                            </div>
                    ...
                    <div className="flex flex-col items-center justify-center py-24 bg-gray-900/50 rounded-3xl border-2 border-dashed border-gray-800">
                        <div className="p-6 bg-gray-800 rounded-full mb-4">
                            <BellOff className="w-12 h-12 text-gray-600" />
                        </div>
                        <h3 className="text-xl font-bold text-gray-200">No new alerts</h3>
