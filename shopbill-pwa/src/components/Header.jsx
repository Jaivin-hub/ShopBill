...
import { Smartphone, User, Bell, Settings, Sun, Moon } from 'lucide-react'; 
...
const Header = ({ 
    ...
    const getButtonClasses = (pageName) => {
        ...
        if (darkMode) {
            return `p-2 rounded-xl transition-all duration-300 active:scale-95 border ${
                isActive 
    ...
    return (
        <header className={`fixed top-0 left-0 right-0 border-b md:hidden z-50 p-4 flex justify-between items-center ${headerBg}`}>
            <div className="flex items-center cursor-pointer group" onClick={() => setCurrentPage('dashboard')}>
                <div className="bg-indigo-600/10 p-1.5 rounded-lg mr-2 border border-indigo-500/20">
