...
const ResetPassword = () => {
    ...
    useEffect(() => {
        ...
        if (urlToken) {
            setToken(urlToken);
            setStatus({ message: 'Ready to set a new password.', type: 'info' });
        } else {
            setStatus({ message: 'Error: Invalid reset link or missing token in the URL.', type: 'error' });
        }
        setLoading(false);
    }, []);
    ...
    const handleResetPassword = async (e) => {
        ...
        if (!token) {
            setStatus({ message: 'Cannot reset: Token is missing from the URL. Please ensure you clicked the full link.', type: 'error' });
            return;
        }
        ...
        const endpoint = `${API.resetpassword}/${token}`; 
        // ----------------------------
        
        setLoading(true);

        try {
            // PUT request to the backend with the new password and confirmation
            ...
            if (apiErrorMessage) {
                ...
                if (apiErrorMessage.toLowerCase().includes('password')) {
                    ...
                    setStatus({ 
                        message: apiErrorMessage || 'The reset link is invalid or has expired. Please request a new one.', 
                        type: 'error' 
                    });
                    setToken(null); 
                }
